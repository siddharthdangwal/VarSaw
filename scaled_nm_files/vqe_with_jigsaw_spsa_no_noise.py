...
def compute_expectations_with_jigsaw(parameters, paulis, shots, meas_size, backend, mode, opt_level, external_dists = None, dict_filename = None, seed = 0):
    ...
    if external_dists == None:
        ...
        shots_per_circuit_global = shots//(2*len(circuits))
    ...
    shots_per_circuit_marginal = shots//(2*len(marginal_circuits))
    ...
    if external_dists == None:
        if mode == 'no_noisy_sim' or mode == 'device_execution':
            job = execute(circuits, backend = backend, optimization_level = 3, shots = shots_per_circuit_global)
            result = job.result()
            ...
            for __, _id in enumerate(paulis):
                if _id == len(_id)*'I':
                    all_counts.append({len(_id)*'0':shots_per_circuit_global})
                else:
    ...
    if mode == 'no_noisy_sim' or mode == 'device_execution':
        job = execute(list(marginal_circuits.values()), backend = backend, shots = shots_per_circuit_marginal, optimization_level = 3)
        result = job.result()
        #all_marginal_counts = job.result().get_counts()
        all_marginal_counts = []
        for __, _id in enumerate(marginal_paulis):
            if _id == len(_id)*'I':
                all_marginal_counts.append({len(_id)*'0':shots_per_circuit_marginal})
            else:
                all_marginal_counts.append(result.get_counts(__))
...
def vqe_with_jigsaw_wrapper(parameters, paulis, coeffs, shots, meas_size, backend, mode = 'no_noisy_sim', opt_level = 1, loss_filename = None, params_filename = None, dict_filename = None, seed = 0):
...
if opt_level == 1:
    ...
    vqe_result_jigsaw = spsa.optimize(num_vars = 2*n_qubits*(p+1), objective_function = lambda c:vqe_with_jigsaw(c, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = 2, backend = simulator, external_dists = None, mode = 'no_noisy_sim', opt_level = 1, loss_filename = loss_filename, params_filename = params_filename, dict_filename = global_dict_name_prefix, seed = seed)[0], variable_bounds = bounds, initial_point = initial_point)
    ...
    loss, reconstructed_counts = vqe_with_jigsaw(initial_point, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = 2, backend = simulator, external_dists = None, mode = 'no_noisy_sim', opt_level = 2, loss_filename = loss_filename, params_filename = params_filename, dict_filename = global_dict_name_prefix, seed = seed)
    external_dists = reconstructed_counts

    vqe_result_jigsaw = spsa.optimize(num_vars = 2*n_qubits*(p+1), objective_function = lambda c:vqe_with_jigsaw_wrapper(c, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = 2, backend = simulator, mode = 'no_noisy_sim', opt_level = 2, loss_filename = loss_filename, params_filename = params_filename, dict_filename = global_dict_name_prefix, seed = seed), variable_bounds = bounds, initial_point = initial_point)
