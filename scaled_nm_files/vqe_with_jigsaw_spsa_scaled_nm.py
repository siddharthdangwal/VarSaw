...
scaled_noise_model = NoiseModel.from_backend(scaled_props)

#get the vqe result
if opt_level == 1:

    loss_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt1_loss_' + sys.argv[1] + '_scaling_factor_' + sys.argv[4] + '.csv'
    params_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt1_params_' + sys.argv[1] + '_scaling_factor_' + sys.argv[4] + '.csv'
    ...
    loss_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt2_loss_' + sys.argv[1] + '_scaling_factor_' + sys.argv[4] + '.csv'
    params_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt2_params_' + sys.argv[1] + '_scaling_factor_' + sys.argv[4] + '.csv'
    global_dict_name_prefix = hamiltonian_string_elements[-1]
    iter_num = 0
    ...
    objective_function = lambda c:vqe_with_jigsaw_wrapper(c, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = 2, backend = device, mode = 'noisy_sim', opt_level = 2, loss_filename = loss_filename, params_filename = params_filename, dict_filename = global_dict_name_prefix, noise_model = scaled_noise_model, seed = seed)
    vqe_result_jigsaw = minimize(objective_function, initial_point, bounds, budget, method = 'imfil')
