...
def quantum_state_preparation(circuit, parameters, entanglement_type, entanglement_maps = None):
    '''
    Args:
    circuit: The input circuit to which we append the parameterized state
    parameters: The parameters of the rotations
    entanglement_type: Determines pattern of CX gates - full, linear, circular, sca, or asymmetric
    entanglement_maps: Specified only in case the ansatz is asymmetric. Specifies which qubits to entangle for every entanglement block
    
    Returns:
    Circuit with ansatz for a generalized state appended to it
    '''
    ...
    if entanglement_type in ['full', 'linear', 'circular', 'sca']:
        ansatz = EfficientSU2(num_qubits = num_qubits, entanglement = entanglement_type, reps = p, insert_barriers = True)
        ansatz.assign_parameters(parameters = parameters, inplace = True)
        circuit.compose(ansatz, inplace = True)
    elif entanglement_type == 'asymmetric':
        ansatz = TwoLocal(num_qubits = num_qubits, rotation_blocks = ['ry', 'rz'], entanglement_blocks = 'cx', entanglement = entanglement_maps, reps = p, insert_barriers = True)
        ansatz.assign_parameters(parameters = parameters, inplace = True)
        circuit.compose(ansatz, inplace = True)
...
def vqe_circuit(n_qubits, parameters, hamiltonian, e_type, entanglement_map = None):
    '''
    Args:
    ...
    circuit = quantum_state_preparation(circuit, parameters, entanglement_type = e_type, entanglement_maps=entanglement_map)
...
def compute_expectations_with_jigsaw(parameters, paulis, shots, meas_size, backend, e_type, entanglement_map, mode, opt_level, external_dists = None, dict_filename = None, seed = 0):
    '''
    Args:
    ...
    if opt_level == 1:
        marginal_paulis = generate_marginal_paulis2(paulis, meas_size)
    else:
    ...
    if external_dists == None:
        circuits = [vqe_circuit(n_qubits, parameters, pauli, e_type, entanglement_map) for pauli in paulis]

    #get all the marginal vqe circuits
    marginal_circuits = [vqe_circuit(n_qubits, parameters, hamil, e_type, entanglement_map) for hamil in marginal_paulis]
...
########################################## Compute VQE loss ###################################################### 
def compute_loss_with_jigsaw(parameters, paulis, coeffs, shots, meas_size, backend, e_type, entanglement_map, mode, opt_level, external_dists, dict_filename = None, seed = 0):
    '''
    Args:
    parameters: The parameters of the VQE ansatz
    paulis: The paulis tensor hamiltonians
    ...
    expectations, reconstructed_counts = compute_expectations_with_jigsaw(parameters, paulis,  shots, meas_size, backend, e_type, entanglement_map, mode, opt_level, external_dists = external_dists, dict_filename = dict_filename, seed = seed)
...
####################################################### VQE function ###################################################################
def vqe_with_jigsaw(parameters, paulis, coeffs, shots, meas_size, backend, e_type, entanglement_map = None, external_dists = None, mode = 'no_noisy_sim', opt_level = 1, loss_filename = None, params_filename = None, dict_filename = None, seed = 0):
    '''
    Args:
    parameters: The parameters of the VQE ansatz
    paulis: The paulis tensor hamiltonians
    coeffs: The coefficients corresponding to each pauli tensor
    backend: The backend on which the vqe is run
    e_type: Specifies the type of ansatz
    entanglement_map: The entanglement arrangement to use in case the ansatz is not symmetric
    external_dists: A dictionary containing distributions for each term in the hamiltonian. If none, then compute
    ...
    loss, reconstructed_counts =  compute_loss_with_jigsaw(parameters, paulis, coeffs, shots, meas_size, backend, e_type, entanglement_map, mode, opt_level, external_dists = external_dists, dict_filename = dict_filename, seed = seed)
...
def vqe_with_jigsaw_wrapper(parameters, paulis, coeffs, shots, meas_size, backend, e_type, entanglement_map = None, mode = 'noisy_sim', opt_level = 1, loss_filename = None, params_filename = None, dict_filename = None, seed = 0):
    
    #get the reconstructed counts and the loss value
    global external_dists
    loss, reconstructed_counts = vqe_with_jigsaw(parameters, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = meas_size, backend = backend, e_type = e_type, entanglement_map = entanglement_map, external_dists = external_dists, mode = mode, opt_level = opt_level, loss_filename = loss_filename, params_filename = params_filename, dict_filename = dict_filename, seed = seed)
...
def callback_function(nfev, x_next, fx_next, norm, boolean, opt_level, loss_list = None, params_list = None, loss_filename = None, params_filename = None, paulis = None, coeffs = None, device = None, seed = None):
    ...
    if opt_level == 5:
        ...
        if not (loss_filename == None):
            with open(loss_filename, 'a') as file:
                print('Loss for iteration number ', current_iteration, ' is: ', fx_next)
                writer = csv.writer(file)
                writer.writerow([fx_next])
...
#get the type of ansatz
ansatz_type = sys.argv[4]
...
#specify the asymmetric ansatz to use in case of asymmetric entanglement type
entanglement_map = None
if n_qubits == 4:
    entanglement_map = [[(0, 2), (1, 2), (0, 1), (2, 3)], [(0, 3), (2, 3), (1, 2)]]
elif n_qubits == 6:
    entanglement_map = [[(0, 3), (1, 3), (2, 5), (1, 2), (0, 3), (3, 4)], [(0, 1), (1, 3), (1, 5), (2, 3), (1, 2), (3, 4)]]
elif n_qubits == 8:
    entanglement_map = [[(0, 1), (1, 3), (4, 5), (2, 5), (6, 7), (3, 7), (0, 2), (1, 4), (5, 7), (3, 5)], [(0, 1), (0, 2), (3, 5), (1, 3), (4, 7), (3, 4), (1, 3), (5, 7), (0, 2), (4, 5), (5, 6), ()]]
...
if opt_level == 1:
    ...
    loss_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt1_loss_' + sys.argv[1] + '_' + ansatz_type + '_.csv'
    params_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt1_params_' + sys.argv[1] + '_' + ansatz_type + '_.csv'
    ...
    objective_function = lambda c:vqe_with_jigsaw(c, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = 2, backend = device, e_type = ansatz_type, entanglement_map = entanglement_map, external_dists = None, mode = 'noisy_sim', opt_level = 1, loss_filename = loss_filename, params_filename = params_filename, dict_filename = global_dict_name_prefix, seed = seed)[0]
    ...
    loss_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt2_loss_' + sys.argv[1] + '_' + ansatz_type + '_.csv'
    params_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt2_params_' + sys.argv[1] + '_' + ansatz_type + '_.csv'
    ...
    loss, reconstructed_counts = vqe_with_jigsaw(initial_point, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = 2, backend = device, e_type = ansatz_type, entanglement_map = entanglement_map, external_dists = None, mode = 'noisy_sim', opt_level = 2, loss_filename = loss_filename, params_filename = params_filename, dict_filename = global_dict_name_prefix, seed = seed)
    external_dists = reconstructed_counts

    #converge second iteration onwards
    objective_function = lambda c:vqe_with_jigsaw_wrapper(c, paulis = paulis, coeffs = coeffs, shots = shots_dev, meas_size = 2, backend = device, e_type = ansatz_type, entanglement_map = entanglement_map, mode = 'noisy_sim', opt_level = 2, loss_filename = loss_filename, params_filename = params_filename, dict_filename = global_dict_name_prefix, seed = seed)
    vqe_result_jigsaw = spsa.optimize(num_vars = 2*n_qubits*(p+1), objective_function = objective_function, variable_bounds = bounds, initial_point = initial_point)

elif opt_level == 3:

    loss_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt3_loss_' + sys.argv[1] + '_' + ansatz_type + '_.csv'
    params_filename = hamiltonian_string_elements[-1] + '_vqe_with_jigsaw_opt3_params_' + sys.argv[1] + '_' + ansatz_type + '_.csv'
